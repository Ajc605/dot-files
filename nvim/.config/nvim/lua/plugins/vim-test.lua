return {
    ...
    config = function()
        ...
        vim.cmd("let test#php#phpunit#executable = 'php bin/phpunit'")
        ...
        vim.keymap.set("n", "<leader>tdb", ":!php bin/console doctrine:database:create --env=test<CR>", { desc = "Create Test DB" })
        vim.keymap.set("n", "<leader>tdm", ":!php bin/console doctrine:migrations:migrate --env=test -n<CR>", { desc = "Migrate Test DB" })
        vim.keymap.set("n", "<leader>tdf", ":!php bin/console doctrine:fixtures:load --env=test -n<CR>", { desc = "Load Test Fixtures" })
    end
}
